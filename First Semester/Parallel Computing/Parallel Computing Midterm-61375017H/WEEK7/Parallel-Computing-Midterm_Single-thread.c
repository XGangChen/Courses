 ...
 int main(void)
 {
     // For reproducibility of random targets
     srand(42);
     // 1) Generate random targets
     double targets[N_WAYPOINTS][3];
     for(int i = 0; i < N_WAYPOINTS; i++) {
         targets[i][0] = (double)rand() / RAND_MAX; // x in [0,1]
         targets[i][1] = (double)rand() / RAND_MAX; // y in [0,1]
         targets[i][2] = (double)rand() / RAND_MAX; // z in [0,1]
     }
 
     // 2) Prepare array to store IK solutions
     double solutions[N_WAYPOINTS][DOF];
 
     // 3) Solve IK for each waypoint, single-threaded
     ...
     // 4) Print a few results to check
     //    (Printing all 1000 might be too much.)
     printf("Single-thread results (first 3 waypoints only):\n");
     for(int i = 0; i < 3; i++) {
     ...
     printf("\nSingle-thread total time for %d waypoints: %.3f s\n",
            N_WAYPOINTS, elapsed);
