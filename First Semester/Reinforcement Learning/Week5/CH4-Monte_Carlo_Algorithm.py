...
class Environment():
    ...
    def step(self, state_coordinates, action): #Input(state, action), Output(next state)
...
class Monte_Carlo():
    ...
    def generate_random_state(self): #Generate random state
        while True:
    ...
    def generate_random_action(self):
        ...
        while (self.env.step(self.current_state_coordinates, action) == self.current_state_coordinates).all():
                action = self.env.direction_dict[np.random.randint(4)]
        return action
    ...
    def policy(self, state_coordinate): #Optimal policy, find the maximun Q(s,a) and return action  
        ...
        for action in self.env.action_dict.keys():
            if not (state_coordinate == self.env.step(np.array(state_coordinate), action)).all():
    ...
    def iterate(self): #Main loop
        for iterration in range(self.Max_iteration):
            episode = []
            self.current_state_coordinates = self.generate_random_state()
            action = self.generate_random_action()
            for h in range(self.Horizon): #Generate episode
                next_state_coordinates = self.env.step(self.current_state_coordinates, action)
                reward = -1
...
if __name__ == "__main__":
    ...
    for state_action in initial_policy:
        MC_Slides.Q_values[state_action] = 1
