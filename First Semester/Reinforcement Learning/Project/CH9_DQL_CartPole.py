...
import FrozenLake_CusEnv_MovPolar
import FrozenLake_CusEnv_polar

# env = gym.make('CartPole-v0', render_mode='rgb_array').unwrapped
env = gym.make('CustomFrozenLake-v1', desc=None, map_name="8x8", is_slippery=False, render_mode='rgb_array')
...
#         # self.conv_layer4 = nn.Conv2d(64, 32, kernel_size=5, stride=2, padding=2)
...
class Deep_Q_Network(nn.Module):
    def __init__(self, frame_height, frame_width, action_count):
        super(Deep_Q_Network, self).__init__()
        self.conv_layer1 = nn.Conv2d(1, 16, kernel_size=5, stride=1, padding=2)  # Add padding
        self.batch_norm1 = nn.BatchNorm2d(16)
        ...
        self.conv_layer3 = nn.Conv2d(32, 64, kernel_size=5, stride=1, padding=2)
        self.batch_norm3 = nn.BatchNorm2d(64)
        self.pool3 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.conv_layer4 = nn.Conv2d(64, 32, kernel_size=3, stride=1, padding=1)  # Smaller kernel
        self.batch_norm4 = nn.BatchNorm2d(32)
        self.pool4 = nn.MaxPool2d(kernel_size=2, stride=2)

        # Compute the flattened size dynamically
        with torch.no_grad():
            dummy_input = torch.zeros(1, 1, frame_height, frame_width)  # Batch size of 1
            self.flattened_input_size = self._get_flattened_size(dummy_input)

        self.final_layer = nn.Linear(self.flattened_input_size, action_count)

    def _get_flattened_size(self, x):
        """Pass a dummy input through conv layers to compute the flattened size dynamically."""
        x = F.relu(self.batch_norm1(self.conv_layer1(x)))
        x = F.relu(self.batch_norm2(self.conv_layer2(x)))
        x = F.relu(self.batch_norm3(self.conv_layer3(x)))
        x = F.relu(self.batch_norm4(self.conv_layer4(x)))

        # x = F.relu(self.pool1(self.batch_norm1(self.conv_layer1(x))))
        # x = F.relu(self.pool2(self.batch_norm2(self.conv_layer2(x))))
        # x = F.relu(self.pool3(self.batch_norm3(self.conv_layer3(x))))
        # x = F.relu(self.pool4(self.batch_norm4(self.conv_layer4(x))))
...
def calculate_cart_screen_position(screen_width):
    total_world_width = env.unwrapped.x_threshold * 2  # Access the unwrapped environment
    screen_to_world_scale = screen_width / total_world_width
    cart_position_in_world = env.unwrapped.state[0]  # Access unwrapped state
...
epsilon = 0.0005 # epsilon greedy exploration-exploitation (higher more random)
EPS_START = 1 
EPS_END = 0.0001
TARGET_UPDATE = 50
BATCH_SIZE = 64
...
# optimizer = optim.SGD(policy_net.parameters())Adagrad
...
def run_demo(policy_net, num_episodes=1):
    ...
    demo_env = gym.make('CustomFrozenLake-v1', desc=None, map_name="8x8", is_slippery=False, render_mode='rgb_array')
...
run_demo(policy_net, num_episodes=20)
env.render()
env.close()
plt.ioff()
plt.show()
