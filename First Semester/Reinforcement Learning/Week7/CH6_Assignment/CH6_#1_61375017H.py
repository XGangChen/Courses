...
reward_matrix = np.array([
    # A   B   C   D   E   F
    [-1, -5, -5, -5, -1, -5], # A (state 0)
    [-5, -1, -3, -1, -5, -3], # B (state 1)
    [-5, -3, -1, -1, -5, -5], # C (state 2)
    [-5, -3, -3, -1, -1, -5], # D (state 3)
    [-1, -5, -5, -3, -1, -1], # E (state 4)
    [-5, -3, -5, -5, -1, -1], # F (state 5)
])
...
def reset_environment():
    global current_state, visited_A
    current_state = 2  # Reset to Room C
    visited_A = False  # Reset the flag
...
def step(action):
    ...
    if current_state == 5 and visited_A:
        reward = 50
    elif current_state == 5 and not visited_A:
        current_state = 0
        reward = -1
    done = (current_state == 5 and visited_A)  # If reached F with reward 50, done
    return current_state, reward, done

# Q-Learning algorithm without using class
def q_learning(alpha=0.1, gamma=0.9, epsilon=0.1, episodes=1000, max_steps=100):
...
def test_agent(q_table, max_steps=100):
    ...
    print(f"Starting at Room C (State {state})")
