...
learning_rate = 0.0001 # e.g. 0.0001 bad learning rate test
...
for i in range(iteration): # learning iterations
    ...
    d_theta2 = (1 / n) * np.sum((y_pred - y) * (x**2).reshape(-1))
    ...
    ax.annotate(f"MSE New: {New_mse:.2f}", xy=(0.05, 0.65), fontsize=10, color='green', xycoords='axes fraction')
    ax.annotate(f"MSE: {mse:.2f}", xy=(0.05, 0.70), fontsize=10, color='red', xycoords='axes fraction')
    ax.annotate(f"new teta_0: {theta0:.6f}", xy=(0.05, 0.75), fontsize=10, xycoords='axes fraction')
    ax.annotate(f"new teta_1: {theta1:.6f}", xy=(0.05, 0.80), fontsize=10, xycoords='axes fraction')
    ax.annotate(f"new teta_2: {theta2:.6f}", xy=(0.05, 0.85), fontsize=10, xycoords='axes fraction')
    ...
    ax.annotate(f"Partial derivative teta_2: {d_theta2:.2f}", xy=(0.05, 1.0), fontsize=10, xycoords='axes fraction')
